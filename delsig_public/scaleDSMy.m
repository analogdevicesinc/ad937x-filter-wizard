function dsm = scaleDSMy( dsm, n_sigma, f, a )
...
%save conventional data globally for future plots
%global Fcon Fcon2_2
% Fcon = F;
% Fcon2_2 = F2_2;
%remove bracket for plots
%{
figure(9); clf
...
for i = 1:order-2
    subplot(order-2,1,i);
    hold on;
    plot(f,M.*a'.*abs(squeeze(F(i,1,:))),'g--');
    %plot(f,n_sigma*sigma_e*abs(squeeze(F(i,2,:))),'r');
    plot(f,M.*a'.*abs(squeeze(F(i,1,:)))+n_sigma*sigma_e*F2_2(i), 'b')
    %plot(f,M.*a'.*abs(squeeze(Fcon(i,1,:)))+n_sigma*sigma_e*Fcon2_2(i),'b-.');%, 'Color',[0.0 0.0 1])
    grid on;
    ylabel(strcat('Amp ',num2str(i)))
    if i==1 
        title('Opamp outputs for 4th-order feedback modulator')
        legend('Signal', 'Signal+Noise', 'Location', 'NorthEast')
    end
